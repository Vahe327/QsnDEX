...
export function VolumeChart({ data = [], height = 250, className, loading = false }: VolumeChartProps) {
  ...
  useEffect(() => {
    ...
    rafId = requestAnimationFrame(() => {
      ...
      const formatted = validData.map((d) => ({
        time: d.time,
        value: d.value,
        color: d.color || 'rgba(240, 180, 41, 0.45)',
      }));
      histogramSeries.setData(formatted as any);
