...
interface MobileAddLiquidityProps {
...
export function MobileAddLiquidity({ initialTokenA, initialTokenB, initialFee, poolAddress }: MobileAddLiquidityProps) {
  ...
  return (
    <div className="px-3 pt-3 pb-4">
      ...
      <div className="flex items-center gap-2.5 mb-4">
        ...
        >
          <Droplets className="w-5 h-5" style={{ color: 'var(--accent-primary)' }} />
        </div>
      ...
      <div className="[&_.card]:!rounded-2xl [&_.card]:!border-[var(--border-subtle)] [&_button]:min-h-[44px]">
        <AddLiquidity initialTokenA={initialTokenA} initialTokenB={initialTokenB} initialFee={initialFee} poolAddress={poolAddress} />
      </div>
    </div>
  );
}
